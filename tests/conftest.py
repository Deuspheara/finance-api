...
async def test_engine():
    logger.debug(
        f"test_engine: Starting fixture, event loop: {id(asyncio.get_running_loop())}"
    )
    ...
    logger.debug(
        f"test_engine: Yielding engine, event loop: {id(asyncio.get_running_loop())}"
    )
    yield engine
    ...
    logger.debug(
        f"test_engine: Disposed engine, event loop: {id(asyncio.get_running_loop())}"
    )
...
async def test_session(test_engine):
    logger.debug(
        f"test_session: Starting fixture, event loop: {id(asyncio.get_running_loop())}"
    )
    async_session = async_sessionmaker(test_engine, expire_on_commit=False)
    session = async_session()
    try:
        logger.debug(
            f"test_session: Created session, event loop: {id(asyncio.get_running_loop())}"
        )
        # Start a transaction for test isolation
        await session.begin()
        logger.debug(
            f"test_session: Yielding session, event loop: {id(asyncio.get_running_loop())}"
        )
        yield session
    finally:
        ...
        await session.close()
        logger.debug(
            f"test_session: Closed session, event loop: {id(asyncio.get_running_loop())}"
        )
...
async def client(test_session):
    logger.debug(
        f"client: Starting fixture, event loop: {id(asyncio.get_running_loop())}"
    )
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        try:
            logger.debug(
                f"client: Yielding client, event loop: {id(asyncio.get_running_loop())}"
            )
            yield client
        finally:
            app.dependency_overrides.clear()
            logger.debug(
                f"client: Closed client and cleared overrides, event loop: {id(asyncio.get_running_loop())}"
            )
