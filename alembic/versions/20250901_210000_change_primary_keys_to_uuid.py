...
def upgrade() -> None:
    ...
    op.drop_constraint(
        "conversationlog_user_id_fkey", "conversationlog", type_="foreignkey"
    )
    ...
    # Change primary keys to UUID
    op.alter_column(
        "user", "id", type_=sa.UUID(), postgresql_using="uuid_generate_v4()"
    )
    op.alter_column(
        "finance", "id", type_=sa.UUID(), postgresql_using="uuid_generate_v4()"
    )
    op.alter_column(
        "subscription", "id", type_=sa.UUID(), postgresql_using="uuid_generate_v4()"
    )
    op.alter_column(
        "usagelog", "id", type_=sa.UUID(), postgresql_using="uuid_generate_v4()"
    )
    op.alter_column(
        "userconsent", "id", type_=sa.UUID(), postgresql_using="uuid_generate_v4()"
    )
    op.alter_column(
        "auditlog", "id", type_=sa.UUID(), postgresql_using="uuid_generate_v4()"
    )
    op.alter_column(
        "conversationlog", "id", type_=sa.UUID(), postgresql_using="uuid_generate_v4()"
    )
    ...
    op.create_foreign_key(
        "auditlog_user_id_fkey", "auditlog", "user", ["user_id"], ["id"]
    )
    op.create_foreign_key(
        "conversationlog_user_id_fkey", "conversationlog", "user", ["user_id"], ["id"]
    )
    op.create_foreign_key(
        "subscription_user_id_fkey", "subscription", "user", ["user_id"], ["id"]
    )
    op.create_foreign_key(
        "usagelog_user_id_fkey", "usagelog", "user", ["user_id"], ["id"]
    )
    op.create_foreign_key(
        "userconsent_user_id_fkey", "userconsent", "user", ["user_id"], ["id"]
    )
